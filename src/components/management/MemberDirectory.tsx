  ...
  return (
    ...
    >
      <div className="flex items-center space-x-4">
        ...
        />
        <div>
          <p className="font-bold text-black group-hover:text-[#d81313] transition-colors">
            {user.name}
          </p>
      ...
      <div className="flex items-center space-x-4">
        {user.onboardingStatus === "Awaiting Verification" && (
          <span className="text-xs font-bold bg-yellow-400 text-black px-2 py-1 hidden sm:block">
            AWAITING VERIFICATION
          </span>
        )}
        <span className="text-sm text-black font-semibold hidden md:block">
          {user.role}
        </span>
      ...
      {filteredMembers.length > 0 ? (
        ...
        <div className="p-8 text-center">
          <p className="text-neutral-500">
            No members found for the current selection.
          </p>
        </div>
      )}
    </div>
