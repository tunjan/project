...
const CreateCubeForm: React.FC<CreateCubeFormProps> = ({
  onCreateEvent,
  onCancel,
}) => {
  // FIX: Use the correct hook to get the current user
  const currentUser = useCurrentUser();
  // FIX: Get all chapter data from the central store
  const allChapters = useChapters();

  if (!currentUser) return null;

  const isHighLevelOrganizer = [
    Role.REGIONAL_ORGANISER,
    Role.GLOBAL_ADMIN,
    Role.GODMODE,
  ].includes(currentUser.role);

  // FIX: Correctly derive the list of chapters the user can organize
  const organizerChapters = isHighLevelOrganizer
      ...
      <div className="max-w-2xl mx-auto bg-white border border-black">
        <div className="p-8 border-b border-black">
          ...
          <p className="mt-2 text-neutral-600">
            Fill in the details below to schedule a new event. It will be
            visible to all members immediately.
          </p>
        </div>
          ...
          <div>
            ...
            {organizerChapters.length === 0 && (
              <p className="text-xs text-neutral-500 mt-1">
                You are not an organiser of any chapter. Contact an admin.
              </p>
            )}
          </div>

          <InputField
            label="Exact Location (e.g., Times Square, Union Square)"
            id="location"
            value={location}
