...
const EditChaptersModal: React.FC<EditChaptersModalProps> = ({ user, allChapters, onClose, onSave }) => {
    const [selectedChapters, setSelectedChapters] = useState<string[]>(user.chapters || []);
        ...
        >
            ...
            >
                <div className="text-center">
                    <h2 className="text-2xl font-extrabold text-black">Edit Chapter Memberships</h2>
                    <p className="mt-2 text-sm text-neutral-600">
                        Select the chapters <span className="font-bold">{user.name}</span> is a member of.
                    </p>
                </div>
                
                <div className="my-6">
                    <div className="space-y-2 border border-black p-4 max-h-64 overflow-y-auto">
                        {allChapters.sort((a,b) => a.name.localeCompare(b.name)).map(chapter => (
