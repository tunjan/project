...
const KeyDisplay: React.FC<{ title: string; value: string | null }> = ({ title, value }) => (
    <div>
...
const SecurityDashboard: React.FC<SecurityDashboardProps> = ({ organizer }) => {
    ...
    const [verificationResult, setVerificationResult] = useState<'valid' | 'invalid' | 'unchecked'>('unchecked');
    ...
    const handleVerifyToken = () => {
        ...
        try {
            ...
            const isValid = verifySignature(parsedToken.payload, parsedToken.signature, parsedToken.publicKey);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white border border-black p-6">
                 <div className="flex items-center border-b border-black pb-4 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-black">Key Management (Simulation)</h3>
                </div>
                <div className="space-y-4">
                    <p className="text-sm text-neutral-600">
                        In a real application, your secret key would be securely stored. For this simulation, you can generate a new keypair. This keypair is not persisted.
                    </p>
                    {keyPair && (
                        <div className="space-y-3 p-4 border border-black bg-neutral-50">
                            <KeyDisplay title="Public Key (base64)" value={keyPair.publicKey} />
                            <KeyDisplay title="Secret Key (base64)" value={keyPair.secretKey} />
                        </div>
                    )}
                    <button
            ...
            <div className="bg-white border border-black p-6">
                ...
                <div className="space-y-4">
                    <p className="text-sm text-neutral-600">
                        Paste the full identity token string (from a QR code) to verify its authenticity.
                    </p>
                    <div>
                        <label htmlFor="token-verify" className="block text-sm font-bold text-black mb-1">Identity Token String</label>
                        <textarea
                            id="token-verify"
                            rows={6}
                            value={tokenToVerify}
                    ...
                    {verificationResult !== 'unchecked' && (
                        <div className={`p-3 border ${
                            ...
                            <p className="font-bold text-sm">{verificationResult === 'valid' ? 'VALID SIGNATURE' : 'INVALID SIGNATURE'}</p>
                            <p className="text-xs">{verificationResult === 'valid' ? 'The token is authentic and has not been tampered with.' : 'The token is not authentic or has been altered.'}</p>
                        </div>
                    )}
