...
const ScopeBadge: React.FC<{ scope: AnnouncementScope, target?: string }> = ({ scope, target }) => {
    ...
    return (
        <span className={`px-2.5 py-1 text-xs font-bold tracking-wider ${bgColor} ${textColor}`}>
            {text}
        </span>
    );
};
    ...
    <div className="bg-white border border-black overflow-hidden">
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
            <ScopeBadge scope={announcement.scope} target={announcement.country || announcement.chapter} />
            <p className="text-sm text-neutral-500">{formattedDate}</p>
        </div>
        
        <h3 className="text-2xl font-bold text-black">{announcement.title}</h3>
        <p className="mt-3 text-neutral-700 whitespace-pre-wrap">{announcement.content}</p>
        ...
        <div className="mt-6 pt-4 border-t border-neutral-200 flex items-center">
          ...
          />
          <div className="ml-3">
            <p className="text-sm font-semibold text-black">{announcement.author.name}</p>
            <p className="text-xs text-neutral-500">{announcement.author.role}</p>
          </div>
        </div>
      </div>
    </div>
