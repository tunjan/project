...
interface EditProfileModalProps {
    user: User;
    onClose: () => void;
    onSave: (updatedData: { name: string; instagram: string, hostingAvailability: boolean, hostingCapacity: number }) => void;
}
...
const EditProfileModal: React.FC<EditProfileModalProps> = ({ user, onClose, onSave }) => {
    ...
    const [hostingAvailability, setHostingAvailability] = useState(user.hostingAvailability);
    const [hostingCapacity, setHostingCapacity] = useState(user.hostingCapacity || 1);

    const handleSubmit = () => {
        onSave({ name, instagram, hostingAvailability, hostingCapacity: Number(hostingCapacity) || 0 });
    };
        ...
        >
            ...
            >
                ...
                <div className="my-6 space-y-4">
                    <InputField label="Full Name" id="edit-name" value={name} onChange={e => setName(e.target.value)} />
                    <InputField label="Instagram Handle" id="edit-instagram" value={instagram} onChange={e => setInstagram(e.target.value)} required={false} />
                    ...
                    <div className="border-t border-black pt-4">
                        <label className="flex items-center space-x-3 cursor-pointer">
                           <input
                            ...
                            />
                            <span className="font-bold text-black">I am available to host other activists</span>
                        </label>
                         {hostingAvailability && (
                             <div className="mt-3 pl-8">
                                <label htmlFor="hosting-capacity" className="block text-sm font-bold text-black mb-1">How many people can you host?</label>
                                <input
                                    type="number"
                                    id="hosting-capacity"
