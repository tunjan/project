...
// This file relies on the global `QRCode` object from the qrcode CDN script.
declare const QRCode: {
    toCanvas: (canvas: HTMLCanvasElement, text: string, options: object, callback: (error?: Error) => void) => void;
};
...
const QrCodeModal: React.FC<QrCodeModalProps> = ({ token, onClose }) => {
    ...
    useEffect(() => {
        if (canvasRef.current) {
            QRCode.toCanvas(canvasRef.current, tokenString, { width: 256, margin: 1 }, (error) => {
        ...
        >
            ...
            >
                <div className="text-center">
                    <h2 className="text-2xl font-extrabold text-black">Verified Identity</h2>
                    <p className="mt-2 text-sm text-neutral-600">Scan this code or copy the text below.</p>
                </div>
                ...
                <div>
                    <label htmlFor="token-string" className="block text-xs font-bold text-black mb-1">Token Data</label>
                    <textarea
                        id="token-string"
                        readOnly
                        rows={4}
