...
import { type Resource, ResourceType, SkillLevel } from '../../types';
...
const ResourcesPage: React.FC<ResourcesPageProps> = () => {
  const { resources: allResources } = useData();
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedType, setSelectedType] = useState<ResourceType | 'all'>('all');
  const [selectedLevel, setSelectedLevel] = useState<SkillLevel | 'all'>('all');
  const [selectedLang, setSelectedLang] = useState('all');

  const languages = useMemo(() => ['all', ...new Set(allResources.map(r => r.language))], [allResources]);
  ...
  const filteredResources = useMemo(() => {
    return allResources
      .filter(resource => {
        ...
        const matchesLevel = selectedLevel === 'all' || resource.skillLevel === selectedLevel;
  ...
  return (
    <div className="py-8 md:py-12">
      <div className="mb-8 md:mb-12">
        <h1 className="text-4xl md:text-5xl font-extrabold text-black tracking-tight">Learning Center</h1>
      ...
      <div className="mt-8">
        {filteredResources.length > 0 ? (
          ...
          <div className="border border-black p-8 text-center bg-white">
            ...
            <h3 className="text-xl font-bold text-black mt-4">No resources found.</h3>
