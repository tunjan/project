        ...
        <div className="border-b border-black last:border-b-0">
            ...
            {isExpanded && (
                <div className="p-4 bg-neutral-50 border-t border-black">
                    <h4 className="text-sm font-bold mb-3">Attendees ({event.participants.length})</h4>
                    <ul className="space-y-3 max-h-48 overflow-y-auto">
                        {event.participants.map(p => (
                            <li key={p.user.id} className="flex items-center space-x-3">
                                <img src={p.user.profilePictureUrl} alt={p.user.name} className="w-8 h-8 object-cover" />
                                <div>
                                    <p className="text-sm font-semibold text-black">{p.user.name}</p>
...
const PastEventsModal: React.FC<PastEventsModalProps> = ({ chapterName, events, onClose }) => {
    return (
        <div 
        ...
        >
            ...
            >
                <div className="p-6 border-b border-black flex justify-between items-center flex-shrink-0">
                    <div>
                        <h2 className="text-2xl font-extrabold text-black">Past Events</h2>
                ...
                <div className="flex-grow overflow-y-auto bg-white">
                   {events.length > 0 ? (
                       ...
                       <div className="p-8 text-center h-full flex items-center justify-center">
                           <p className="text-neutral-500">No past events found for this chapter.</p>
                       </div>
                   )}
                </div>
            </div>
