...
// A helper function to generate sensible axis ticks
...
export const BaseChart: React.FC<BaseChartProps> = ({
    ...
    const chartHeight = height - padding; // Y-axis starts from bottom padding

    const dataMaxValue = Math.max(...data.map(d => d.value), 0);
    const yAxisTicks = generateAxisTicks(dataMaxValue, 4);
    const axisMaxValue = yAxisTicks[yAxisTicks.length - 1] || 1; // Avoid division by zero

    const yScale = (value: number) => padding + chartHeight - ((value / axisMaxValue) * chartHeight);
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full font-sans">
            ...
            {children({ xScale, yScale, chartWidth, chartHeight, maxValue: axisMaxValue })}
        </svg>
    );
};