...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  const { users } = useData();
  const loginableUsers = users.filter(u => u.onboardingStatus === 'Confirmed' || u.onboardingStatus === 'Awaiting Verification');
                ...
                >
                    ...
                    <div className="flex-shrink-0 flex flex-col items-end space-y-1">
                        {hasOrganizerRole(user) && (
                            <span className="text-xs font-bold bg-[#d81313] text-white px-2 py-1">ORGANIZER</span>
                        )}
                        {user.onboardingStatus === 'Awaiting Verification' && (
                            <span className="text-xs font-bold bg-yellow-400 text-black px-2 py-1">PENDING VERIFICATION</span>
                        )}
                    </div>
                </button>
            ))}
