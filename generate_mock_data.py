...
# 1. Generate Chapters
# Use more realistic city names instead of completely random ones
REALISTIC_CITIES = [
    ...
    "Leeds", "Liverpool", "Glasgow", "Bristol", "Newcastle", "Sheffield", "Leicester",
    "San Francisco", "Los Angeles", "Chicago", "Boston", "Seattle", "Portland", "Denver",
    "Austin", "Miami", "Atlanta", "Nashville", "New Orleans", "Las Vegas", "Phoenix",
    "San Diego", "Sacramento", "Oakland", "Fresno", "Bakersfield", "Stockton", "Modesto"
]

chapters = [{
    'name': random.choice(REALISTIC_CITIES),
...
# Ensure unique chapter names
seen_names = set()
for chapter in chapters:
    while chapter['name'] in seen_names:
        chapter['name'] = random.choice(REALISTIC_CITIES)
    seen_names.add(chapter['name'])

print(f"Generated {len(chapters)} chapters with realistic names.")
...
for chapter in chapters:
    ...
    for _ in range(CONFIG['events_per_chapter']):
        # Generate events with more realistic date ranges:
        # - Past events: 1 year ago to 1 week ago (for participation history)
        # - Current/upcoming: 1 week ago to 1 week in the future (max)
        if random.random() < 0.8:  # 80% chance of past events
            dt = get_random_date(now - timedelta(days=365), now - timedelta(days=7))
        else:  # 20% chance of current/upcoming events
            dt = get_random_date(now - timedelta(days=7), now + timedelta(days=7))
...
for _ in range(CONFIG['special_regional_events']):
    ...
    # Regional events should also be more realistic - max 1 week in the future
    start_dt = get_random_date(now - timedelta(days=7), now + timedelta(days=7))
...
import {{
  User, Chapter, CubeEvent, Announcement, Resource, OutreachLog, AccommodationRequest, EventComment, Challenge, Notification, BadgeAward
}} from './types';

{write_data_to_file(users, 'users', 'User')}
{write_data_to_file(chapters, 'chapters', 'Chapter')}
{write_data_to_file(events_to_serialize, 'cube_events', 'CubeEvent')}
{write_data_to_file(announcements_to_serialize, 'announcements', 'Announcement')}
{write_data_to_file(resources, 'resources', 'Resource')}
{write_data_to_file(outreach_logs, 'outreach_logs', 'OutreachLog')}
{write_data_to_file(accommodation_requests_to_serialize, 'accommodation_requests', 'AccommodationRequest')}
{write_data_to_file(event_comments_to_serialize, 'event_comments', 'EventComment')}
{write_data_to_file(challenges, 'challenges', 'Challenge')}
{write_data_to_file(notifications_to_serialize, 'notifications', 'Notification')}
{write_data_to_file(badge_awards_to_serialize, 'badge_awards', 'BadgeAward')}
"""
