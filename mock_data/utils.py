...
class DataSerializer:
    ...
    def write_typescript_file(data: Dict[str, List[Dict[str, Any]]], output_path: str):
        ...
        def write_data_export(data_list: List[Dict[str, Any]], name: str, type_name: str) -> str:
            json_str = json.dumps(data_list, cls=DateTimeEncoder, indent=2)
            return f"export const MOCK_{name.upper()}: {type_name}[] = {json_str};"
...
import {
  ...
  BadgeAward, InventoryItem, ChapterJoinRequest
} from './types';
...
def calculate_data_stats(data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    for key, items in data.items():
        stats[key] = {
            'count': len(items),
            'size_kb': len(json.dumps(items, cls=DateTimeEncoder)) / 1024
        }
